...
# Define a simpler find/replace for the map block
# Find the start and end of the map block using regex indices
pattern = re.compile(r'\{filteredProducts\.map\(product => \{.*?\}\)\}', re.DOTALL)

new_map = '''{filteredProducts.map(product => {
...
if pattern.search(content):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(\"Successfully update ProductGrid.tsx\")
else:
