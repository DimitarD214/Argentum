  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between border-b border-black/5 pb-6">
        <div>
          <h2 className="heading-luxury text-xl tracking-widest uppercase text-astera-900">VAŠA KOŠARICA</h2>
      ...
      <div className="space-y-6">
        {items.map((item) => (
          <div key={`${item.id}-${item.metal}`} className="flex items-center gap-6 group py-4 border-b border-slate-50 last:border-0">
            <div className="relative w-24 h-28 bg-luxury-beige rounded-sm overflow-hidden flex-shrink-0">
              {item.image ? (
                <Image 
                  src={item.image} 
                  alt={item.name} 
            ...
            <div className="flex-grow flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div>
                <h3 className="heading-luxury text-sm tracking-wide text-foreground mb-1">{item.name}</h3>
      ...
      <div className="bg-slate-50 border border-slate-100 rounded-lg p-8 mt-12 flex items-center justify-between">
        <div>
           <span className="heading-luxury text-sm tracking-widest uppercase block text-astera-900">Ukupno</span>
